            ...
            <div className="flex-1 overflow-y-auto">
              {loading ? (
                ...
                <div className="p-8 text-center">
                  ...
                  <p className="text-gray-700 font-semibold text-sm">No doctors yet</p>
                  <p className="text-gray-500 text-xs mt-1">Upload a report to get connected</p>
                </div>
              ) : (
                doctors.map((doctor) => (
                  <button
                    ...
                    className={`w-full p-4 flex items-center gap-3 hover:bg-blue-50 transition-all border-b border-gray-100 ${
                      selectedDoctor?._id === doctor._id ? 'bg-blue-50 border-l-4 border-l-blue-600' : ''
                    }`}
                  >
                    <div className="relative">
                      <div className="w-12 h-12 bg-gradient-to-br from-blue-400 to-blue-600 rounded-full flex items-center justify-center text-xl shadow-md">
                        👨‍⚕️
                      </div>
                    ...
                    <div className="flex-1 text-left min-w-0">
                      ...
                      {doctor.specialization && (
                        <p className="text-[11px] text-blue-600 font-medium mb-0.5">{doctor.specialization}</p>
                      )}
                      {doctor.lastMessage && (
                        <p className="text-xs text-gray-600 truncate">{doctor.lastMessage}</p>
                      )}
                    </div>
            ...
            {selectedDoctor ? (
              ...
              >
                <div className="text-center">
                  ...
                  <h3 className="text-2xl font-semibold text-gray-800 mb-2">Patient Chat</h3>
                  <p className="text-gray-600">Select a doctor to start messaging</p>
                </div>
              </div>
            )}
          </div>
